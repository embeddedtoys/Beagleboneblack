/*
  ...
  Microchip MCP23S17 SPI I/O Expander Class
...
class MCP {
  public:
	SPIClass *pSPI;
    MCP(unsigned char);                            // Constructor to instantiate a discrete IC as an object, argument is the address (0-7) of the MCP23S17
    void wordWrite(__u8, unsigned int);   // Typically only used internally, but allows the user to write any register pair if needed, so it's public
    void byteWrite(__u8, __u8);        // Typically only used internally, but allows the user to write any register if needed, so it's public
    void pinMode(__u8, __u8);          // Sets the mode (input or output) of a single I/O pin
    void pinMode(unsigned int);              // Sets the mode (input or output) of all I/O pins at once
    void pullupMode(__u8, __u8);       // Selects internal 100k input pull-up of a single I/O pin
    void pullupMode(unsigned int);           // Selects internal 100k input pull-up of all I/O pins at once
    void inputInvert(__u8, __u8);      // Selects input state inversion of a single I/O pin (writing 1 turns on inversion)
    void inputInvert(unsigned int);          // Selects input state inversion of all I/O pins at once (writing a 1 turns on inversion)
    void digitalWrite(__u8, __u8);     // Sets an individual output pin HIGH or LOW
    ...
    __u8 digitalRead(__u8);            // Reads an individual input pin
    __u8 byteRead(__u8);               // Reads an individual register and returns the byte. Argument is the register address
    unsigned int digitalRead(void);          // Reads all input  pins at once. Be sure it ignore the value of pins configured as output!
    void AddressEnable(SPIClass *SPIp);
  private:
    __u8 _address;                        // Address of the MCP23S17 in use
